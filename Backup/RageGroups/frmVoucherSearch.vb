Public Class frmVoucherSearch
    ...
    Private Sub frmVoucherSearch_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If GMod.staff1 = 1 Then
            GMod.DataSetRet("select * from vtype where cmp_id='" & GMod.Cmpid & "' and vtype not in ('BANK SAL TRANSFER') order by seqorder", "vty")
            cmbcptype.DataSource = GMod.ds.Tables("vty")
            cmbcptype.DisplayMember = "vtype"
        Else
            GMod.DataSetRet("select * from vtype where cmp_id='" & GMod.Cmpid & "' and vtype not in ('SALARY JOURNAL','BANK SAL TRANSFER')  and Vou_no_seq not in ('99') order by seqorder", "vty")
            cmbcptype.DataSource = GMod.ds.Tables("vty")
            cmbcptype.DisplayMember = "vtype"
        End If
