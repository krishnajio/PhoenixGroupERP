...
Public Class frmdayBookAuth
    ...
    Private Sub frmBook_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If GMod.role.ToUpper = "VIEWER LEVEL-1" Or GMod.role.ToUpper = "ADMIN" Then
            GMod.DataSetRet("select vtype from Vtype where cmp_id='" & GMod.Cmpid & "' and vtype not in ('OPEN')  and Vou_no_seq not in ('99') order by seqorder", "grp")
        Else
            GMod.DataSetRet("select vtype from Vtype where cmp_id='" & GMod.Cmpid & "' and vtype not in ('OPEN')  and Vou_no_seq='" & GMod.Dept & "'  order by seqorder", "grp")
        End If
        ...
        If GMod.Dept = 0 Then
            If GMod.Cmpid <> "PHCH" Then
                sql = "select vtype from Vtype where cmp_id='" & GMod.Cmpid & "' and vou_no_seq='" & GMod.Dept & "' and vtype not like '%SALE%'"
        ...
        ElseIf GMod.Dept = 1 Then
            If GMod.Cmpid <> "PHCH" Then
                sql = "select vtype from Vtype where cmp_id='" & GMod.Cmpid & "' and vou_no_seq='" & GMod.Dept & "'" ' and vtype like '%pur journal%'"
        ...
        Try
            ...
            Else
                ...
                sqlinst = " insert into daybook(cmpid, Acc_code, Acc_name, Narration, Group_name, Sub_Group, Vtype, Vouno, " _
                          ...
                          & GMod.VENTRY & " where vou_no between '" & txtFrom.Text & "' and '" & txtTo.Text & "' and Pay_mode='-' order by cast(vou_no as numeric(18,0)),Entry_id"
                GMod.SqlExecuteNonQuery(sqlinst)
            End If
            '----------------------------------------------------------------------------
