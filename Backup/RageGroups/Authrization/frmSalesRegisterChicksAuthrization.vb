    ...
    Private Sub btnshow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnshow.Click
        ...
        Try
            ...
            sql2 = " insert into salereg ( invno, invdate, hdate, code, cname, qty, free," _
                    & " invamt, neccamt,prd_name, Rate, area, uanme,vou_type,prdunit ,mortality) " _
                    & " select VOU_NO, BillDate,HatchDate,AccCode,h.account_head_name, " _
                    & " Qty,FreeQty,Amount,NeccAmount,ProductName,Rate,Station,'" & GMod.username & "',p.vou_type,PrdUnit,mortality from Printdata p " _
                    ...
                    & " between " & txtCrNoFrom.Text & " and " & txtCrNoTo.Text & " and authr='-' and (FreeQty<>0 OR ProductName in (select ItemName from itemmaster where p.cmp_id='" & GMod.Cmpid & "') " _
                    & " OR ProductName='INVOICE CANCELLED') AND p.CMP_ID='" & GMod.Cmpid & "' AND SESSION = '" & GMod.Session & "' " _
        ...
        GMod.DataSetRet("select invno, CONVERT(VARCHAR(11), invdate, 103) invdate ,CONVERT(VARCHAR(11), hdate, 103)  hdate, code, cname, prd_name, qty, free,  qty + free total ,rate, invamt, neccamt, prdunit,mortality  from salereg  where vou_type='" & voutype.Text & "' and uanme='" & GMod.username & "' order by id  ", "SaleRegGrid")
