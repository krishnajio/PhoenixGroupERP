...
Public Class frmSaleInvoiceNewCalculation
    ...
    Private Sub dgSaleVoucher_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgSaleVoucher.CellEndEdit
        ...
        If e.ColumnIndex = 5 Or e.ColumnIndex = 2 Or e.ColumnIndex = 3 Then
            ...
            tqty = Val(dgSaleVoucher(2, dgSaleVoucher.CurrentCell.RowIndex).Value)
            'MsgBox(fr)
            If GMod.Cmpid = "PHHA" Then
                If Val(txtMortality.Text) <= fr Then
                    fr = fr - Val(txtMortality.Text)
                    'rqty = Val(txtMortality.Text) - Math.Round(fr, 0)
                    'qty1 = Val(dgSaleVoucher(2, dgSaleVoucher.CurrentCell.RowIndex).Value) - rqty
                    dgSaleVoucher(6, dgSaleVoucher.CurrentCell.RowIndex).Value = Math.Round(fr, 0)
                    qty1 = Val(dgSaleVoucher(2, dgSaleVoucher.CurrentCell.RowIndex).Value) - (Math.Round(fr, 0) + Val(txtMortality.Text))
                    bqty = qty1
                    ...
                    rqty = Val(txtMortality.Text)
                Else
                    'rqty = Val(txtMortality.Text) - Math.Round(fr, 0)
                    qty1 = Val(dgSaleVoucher(2, dgSaleVoucher.CurrentCell.RowIndex).Value) - Val(txtMortality.Text)
                    bqty = qty1
                    dgSaleVoucher(6, dgSaleVoucher.CurrentCell.RowIndex).Value = fr
                    dgSaleVoucher(4, dgSaleVoucher.CurrentCell.RowIndex).Value = qty1 * Val(dgSaleVoucher(3, dgSaleVoucher.CurrentCell.RowIndex).Value)
                    rqty = Val(txtMortality.Text) - Math.Round(fr, 0)
                End If

                dgSaleVoucher(7, dgSaleVoucher.CurrentCell.RowIndex).Value = Val(txtMortality.Text)
    ...
    Private Sub dgSaleVoucher_CellEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgSaleVoucher.CellEnter
        ...
        '    If GMod.Cmpid = "PHHA" Then
        '        'If Val(txtMortality.Text) < freeqty Then
        '        '    freeqty = freeqty - Val(txtMortality.Text)
        '        'Else
        ...
        If MessageBox.Show("Are U sure?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) = Windows.Forms.DialogResult.Yes Then
            ...
            Try
                ...
                For i = 0 To dgSaleVoucher.Rows.Count - 1
                    ...
                    narration = narration & "TO INV NO." & txtinvoiceno.Text & " QTY " & bqty & " + " & dgSaleVoucher(6, i).Value & "@ " & dgSaleVoucher(3, i).Value
                    nar = nar & narration
        ...
        If GMod.Cmpid = "PHHA" Then
            ...
            If btnsave.Enabled = True Then
                ...
                If voutype.Text = "SALE CR" Then
                    ...
                    Me.pcname.DisplayMember = "ItemName"
                End If
                If voutype.Text = "SALE WBH-1" Then
                    sql = "select * from ItemMaster where Cmp_id='" & GMod.Cmpid & "' and prdtype='CHICKS' and  ItemName like '%BROILER%'  order by ItemName "
                    GMod.DataSetRet(sql, "Item1")
                ...
                If voutype.Text = "SALE WBH-1" Then
                    ...
                    Me.pcname.DisplayMember = "ItemName"
                End If
                If voutype.Text = "SALE HAJIPUR" Then
                ...
                If voutype.Text = "SALE VARANASI" Then
                    sql = "select * from ItemMaster where Cmp_id='" & GMod.Cmpid & "' and prdtype='CHICKS' order by ItemName "
                    GMod.DataSetRet(sql, "Item1")
                ...
                End If
                'SALE RAIPUR
                If voutype.Text = "SALE RAIPUR" Then
                    sql = "select * from ItemMaster where Cmp_id='" & GMod.Cmpid & "' and prdtype='CHICKS' order by ItemName "
                    GMod.DataSetRet(sql, "Item1")
