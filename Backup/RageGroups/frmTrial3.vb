...
Public Class frmTrial3
    ...
    Private Sub btnshow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnshow.Click
        If cmbSubGroup.Items.Count = 0 Then
            ...
            End If
        End If
        If GMod.username = "rozleen" Then
            If Dr.Checked = False Then
                MsgBox("Please Selcr Dr")
                Me.Close()
            End If
        End If
        ...
        Else
            param = "GROUP/SUB GROUP: " & cmbgrpname.Text & "/" & cmbSubGroup.Text
        End If
        ...
        If chkadvformat.Checked = True Then
            ...
            r.SetParameterValue("grp", "GROUP/SUB GROUP: " & cmbgrpname.Text & "/" & cmbSubGroup.Text)
        ...
        If chksimpleformat.Checked = True Then
            ...
            r1.SetParameterValue("s1", "Trial Balance as on " & dtfrom.Text)
    ...
    Public Sub AllGroup()
        If rdwithopening.Checked = True Then
           ...
           & " from " & GMod.VENTRY & " " _
           & " where vou_date between '" & dtfrom.Value.ToShortDateString & "' and '" & dtTo.Value.ToShortDateString & "' and Pay_mode<>'-' " _
    ...
    Public Sub GroupWise()
        ...
        If rdwithopening.Checked = True Then
           ...
           & " Right Join " _
           & " ( select account_code,account_head_name acname ,group_name, isnull(opening_dr,0) odr  , " _
           & " isnull(opening_cr,0) ocr from " & GMod.ACC_HEAD & " where group_name='" & cmbgrpname.Text & "'  and sub_group_name='" & cmbSubGroup.Text & "') q " _
           & " on p.acc_head_code=q.account_code  " _
           & " where ((isnull(p.dramt,0) + q.odr) <> (isnull(p.cramt,0) + q.ocr))  "
        End If
    ...
    Private Sub cmbgrpname_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbgrpname.LostFocus
        Dim q As String
        q = "select distinct sub_group_name  from " & GMod.ACC_HEAD & "  where group_name ='" & cmbgrpname.Text & "'"
        GMod.DataSetRet(q, "subgrp")

        cmbSubGroup.DataSource = GMod.ds.Tables("subgrp")
    ...
    Private Sub cmbSubGroup_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbSubGroup.Enter
        Dim q As String
        q = "select distinct sub_group_name  from " & GMod.ACC_HEAD & "  where group_name ='" & cmbgrpname.Text & "'"
        GMod.DataSetRet(q, "subgrp")
