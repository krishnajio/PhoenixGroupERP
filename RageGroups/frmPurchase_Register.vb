Imports System.IO
Public Class frmPurchase_Register
    ...
    Private Sub btnshow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnshow.Click
        ...
        sql1 = " SELECT  narration,a.account_head_name name ,a.remark2 rm ,a.address addr,v.vou_no,v.pay_mode FROM " & GMod.VENTRY & " v " _
               & " right join " & GMod.ACC_HEAD & " a on v. Acc_head_code=a.account_code " _
             & " WHERE vou_type='PURCHASE' and cast(vou_no as bigint) between " & txtCrNoFrom.Text & " and " & txtCrNoTo.Text & " and v.group_name in ('PARTY','INTERNAL PARTY')  order by cast(vou_no as bigint)  "
        ...
        If GMod.Cmpid = "PHLU" Or GMod.Cmpid = "PHBU" Then
            sql2 = " select  Vou_no,Acc_head,dramt from " & GMod.VENTRY _
                           & " where Vou_type='PURCHASE' and cast(vou_no as bigint) between " & txtCrNoFrom.Text & " and " & txtCrNoTo.Text & "  and Group_name like '%PURCHASE%' " _
        ...
        Dim pr As String = ""
        For i = 0 To GMod.ds.Tables("sql2").Rows.Count - 1
            If pr <> Val(GMod.ds.Tables("sql2").Rows(i)("Vou_no")) Then
                chk = 0
                dgPurchase.Rows.Add()
                arr = GMod.ds.Tables("sql1").Rows(c)("narration").ToString.Split(" ")
                dgPurchase(0, r).Value = arr(2)
                If GMod.ds.Tables("sql1").Rows(c)("Pay_mode") <> "R" Then
                    dgPurchase(1, r).Value = GMod.ds.Tables("sql1").Rows(c)("name")
                    dgPurchase(3, r).Value = GMod.ds.Tables("sql2").Rows(i)("Acc_head")
                Else
                    dgPurchase(1, r).Value = "R"
                    dgPurchase(3, r).Value = "Entry Reversed"
                End If
                ...
                sqlexp = " select dramt,cramt from " & GMod.VENTRY & " where Vou_type='PURCHASE' and cast(vou_no as bigint) =" & GMod.ds.Tables("sql2").Rows(i)("Vou_no").ToString _
                         & " and Group_name like  '%EXPENSES%'  ORDER BY dramt desc"
                GMod.DataSetRet(sqlexp, "exp")

                If GMod.ds.Tables("exp").Rows.Count = 1 Then
                ...
                Else
                    'for ct and freight
                    dgPurchase(5, r).Value = GMod.ds.Tables("exp").Rows(1)("cramt")
                    'MsgBox(dgPurchase(5, r).Value)
                    dgPurchase(6, r).Value = GMod.ds.Tables("exp").Rows(0)("dramt")
                End If
                ...
                sqlexp = " select cramt from " & GMod.VENTRY & " where Vou_type='PURCHASE' and cast(vou_no as bigint) =" & GMod.ds.Tables("sql2").Rows(i)("Vou_no").ToString _
                         & " and Group_name in ('PARTY','INTERNAL PARTY') "
                ...
                Else
                    dgPurchase(1, r).Value = "Entry Reversed"
                End If
                dgPurchase(2, r).Value = GMod.ds.Tables("sql1").Rows(c)("rm")
                r = r + 1
                c = c + 1
                pr = Val(GMod.ds.Tables("sql2").Rows(i)("Vou_no"))
            Else

                dgPurchase(3, r - 1).Value = GMod.ds.Tables("sql2").Rows(i)("Acc_head")
                dgPurchase(4, r - 1).Value = GMod.ds.Tables("sql2").Rows(i)("dramt")
                pr = Val(GMod.ds.Tables("sql2").Rows(i)("Vou_no"))
                If pr = Val(GMod.ds.Tables("sql2").Rows(i)("Vou_no")) Then
                    dgPurchase.Rows.Add()
                    r = r + 1
    ...
    Private Sub frmPurchase_Register_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Try
            File.Delete("c:\pur_reg.txt")
        Catch ex As Exception
            'MsgBox(ex.Message)
        End Try
    End Sub
    ...
    Private Sub btndosprint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btndosprint.Click
        ...
        Dim frAmount As Double
        Dim vatcatAmount As Double
        Dim prtAmount As Double
        ...
        For I = 0 To dgPurchase.Rows.Count - 1
            ...
            frAmount = frAmount + dgPurchase(5, I).Value
            vatcatAmount = vatcatAmount + dgPurchase(6, I).Value
            prtAmount = prtAmount + dgPurchase(7, I).Value
        ...
        For I = 0 To 60
            s = s + " "
        Next
        sw.WriteLine(s & "TOTAL PURCHASE IN AMOUNT Rs " & Format(totalamt, "0.00").PadRight(9, " "))
        sw.WriteLine(s & "TOTAL FREIGHT IN AMOUNT Rs " & Format(frAmount, "0.00").PadLeft(9, " "))
        sw.WriteLine(s & "TOTAL VAT/CST IN AMOUNT Rs " & Format(vatcatAmount, "0.00").PadRight(9, " "))
        sw.WriteLine(s & "TOTAL PARTY IN AMOUNT Rs " & Format(prtAmount, "0.00").PadRight(9, " "))
