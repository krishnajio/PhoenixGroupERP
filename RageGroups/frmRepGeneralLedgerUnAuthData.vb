...
Public Class frmRepGeneralLedgerUnAuthData
    ...
    Private Sub btnshow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnshow.Click
        Try
            ...
            If RadioButton1.Checked Then
                ...
                & " where vou_date between '" & dtfrom.Value.ToShortDateString & "' and '" & dtto.Value.ToShortDateString & "' and Acc_head_code='" & cmbacheadcode.Text & "' and vou_type<>'BANKREC' and left(Uname,1)<>'$' order by vou_date,vou_no"
                GMod.SqlExecuteNonQuery(sqlsave)
            Else
                ...
                & " from " & GMod.VENTRY & " where vou_date between '" & dtfrom.Value.ToShortDateString & "' and '" & dtto.Value.ToShortDateString & "' and Acc_head_code='" & cmbacheadcode.Text & "' and vou_type<>'BANKREC' and left(Uname,1)<>'$' order by vou_date,vou_no,Entry_id"
                GMod.SqlExecuteNonQuery(sqlsave)
            End If
            '------------------------------------------------------------------------------------------------
            ...
            If boolall = False Then
                ...
                If RadioButton1.Checked Then
                    ...
                    r.SetParameterValue("unauth", "UNAUTHRISED LEDGER")
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            For z = 0 To chklistled.CheckedItems.Count - 1
                ...
                End Try
                sqlopen = "select isnull(sum(dramt),0) - isnull(sum(cramt),0)  from " _
                          & " " & GMod.VENTRY & " where Acc_head_code='" & codefor10 & "' and  vou_date<'" & dtfrom.Text & "' and vou_type<>'BANKREC' "
                GMod.DataSetRet(sqlopen, "opening")
                opnbal = CDbl(GMod.ds.Tables("opening").Rows(0)(0))
                Try
                    GMod.ds.Tables("opening").Clear()
                ...
                If RadioButton1.Checked Then
                    sqlsave = "insert into repGeneralLedger1(cmpid, Acc_head_code, Narration, Vou_no, Vou_date,  vou_type, dramt, cramt, Cheque_no, Uname) " _
                    & " select Cmp_id,Acc_head_code,Narration,Vou_no,Vou_date,Vou_type,dramt,cramt,Cheque_no ,'" & GMod.username & "' from " & GMod.VENTRY _
                    & " where vou_date between '" & dtfrom.Value.ToShortDateString & "' and '" & dtto.Value.ToShortDateString & "' and Acc_head_code='" & codefor10 & "' and vou_type<>'BANKREC' and left(Uname,1)<>'$'  order by vou_date,vou_no"
                    GMod.SqlExecuteNonQuery(sqlsave)
                Else
                    sqlsave = "insert into repPartyLedger(Cmpid, Uname,  Vou_no, Vou_type, Vou_date, Acc_head_code, Acc_head," _
                    & " dramt, cramt, Pay_mode, Cheque_no, Narration, Group_name, Sub_group_name, Ch_issue_date, Ch_date)" _
                    & " select Cmp_id,'" & GMod.username & "',vou_no,vou_type,vou_date,Acc_head_code,acc_head,dramt,cramt,pay_mode,Cheque_no," _
                    & " Narration, group_name, sub_group_name, ch_issue_date, ch_date " _
                    & " from " & GMod.VENTRY & " where vou_date between '" & dtfrom.Value.ToShortDateString & "' and '" & dtto.Value.ToShortDateString & "' and Acc_head_code='" & codefor10 & "' and vou_type<>'BANKREC'  and left(Uname,1)<>'$'  order by vou_date,vou_no,Entry_id"
                    GMod.SqlExecuteNonQuery(sqlsave)
                End If
                '----------------------------------------------------------------------------------------
                'Deleyting Zero Valus
