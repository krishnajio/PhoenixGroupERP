...
Public Class frmCashBook1
    ...
    Private Sub frmBook_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        listgrp.Items.Clear()
        GMod.DataSetRet("select vtype from Vtype where cmp_id='" & GMod.Cmpid & "' and vtype in ('CASH','CR VOUCHER','COLL-AREA','JOURNAL','CR VOUCHER-TR','TRANSFER','CASH CR') and session ='" & GMod.Session & "' order by seqorder", "grp")
    ...
    Private Sub btnshow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnshow.Click
        ...
        Try
            ...
            Dim sqldaybefore As String = "select isnull(sum(dramt),0),isnull(sum(cramt),0) " _
                                         & "from " & GMod.VENTRY & " where vou_date < '" & dtfrom.Value.ToShortDateString() & "' and vou_type<>'BANKREC' and vou_type in(" & lst & ")"
            ...
            sqldaybook = " select Acc_code, Acc_name, Narration, d.Vtype, Vouno, dramt, cramt, [Cheque No], " _
                         ...
                         & " left join vtype v  on d.vtype=v.vtype  and d.cmpid=v.Cmp_id where uname='" & GMod.username & "' and v.cmp_id='" & GMod.Cmpid & "'  and v.session ='" & GMod.Session & "' or (d.vtype='OPEN' and v.session ='" & GMod.Session & "') order by id"
    ...
    Sub header()
        pageno = pageno + 1
        Dim i As Integer
        'sw.WriteLine(Convert.ToChar(27).ToString & Convert.ToChar(67).ToString & Convert.ToChar(72).ToString)
        'sw.WriteLine("")
        Dim s As String
        For i = 0 To 50
            s = s + " "
