Public Class frmPendlingList
    ...
    Sub fillgrid()
        Dim sql As String
        sql = "select   Vou_no, Vou_type, convert(varchar,Vou_date,103) Vou_date ,account_code Acc_head_code, account_head_name Acc_head, Narration, dramt, cramt, " _
              & " a.Group_name,pay_mode from " & GMod.VENTRY & " v inner join " & GMod.ACC_HEAD & " a on v.Acc_head_code = a.account_code  where  pay_mode ='-'  and vou_type in ( select vtype from vtype where cmp_id='" & GMod.Cmpid & "' and vou_no_seq ='" & GMod.Dept & "' ) order by vou_type,CAST(VOU_NO AS BIGINT)"
