Public Class frmItemMasterOther
    ...
    Public Sub fillgrid()
        ...
        sqlselectall = "select * from ItemMasterOther where Cmp_id='" & GMod.Cmpid & "'" ' and prdtype ='" & p & "'"
        GMod.DataSetRet(sqlselectall, "ItemMasterOther")
        dgItemMaster.DataSource = ds.Tables("ItemMasterOther")
    End Sub
    ...
    Private Sub btnsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsave.Click, Button5.Click
        ...
        If txtitemname.Text = "" Then
            ...
            Exit Sub
        End If
        Sqlsavestr = "Insert into ItemMasterOther(ItemName, Rate, Unit, Cmp_id, discount, necc, freeqtyper,prdtype,CAT) "
        Sqlsavestr &= " values ("
    ...
    Private Sub txtitemname_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtitemname.TextChanged
        Dim sqlselectall As String
        sqlselectall = "select * from ItemMasterOther where Cmp_id='" & GMod.Cmpid & "' and ItemName like '%" & txtitemname.Text & "%'" ' AND PRDTYPE ='" & p & "'"
        GMod.DataSetRet(sqlselectall, "ItemMasterOther")
        dgItemMaster.DataSource = ds.Tables("ItemMasterOther")
    End Sub
    Dim p As Integer = 0
    Private Sub rdForPurchse_CheckedChanged(sender As Object, e As EventArgs) Handles rdForPurchse.CheckedChanged
        If rdForPurchse.Checked = True Then
            p = 1
            'fillgrid()
        End If
    End Sub

    Private Sub rdOther_CheckedChanged(sender As Object, e As EventArgs) Handles rdOther.CheckedChanged
